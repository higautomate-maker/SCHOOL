#!/usr/bin/env bash
# Builds the three signed Google Play production bundles. It never uploads them.
set -euo pipefail
...
API_BASE_URL="${HIG_PRODUCTION_API_URL:-https://school.higaai.com}"
LIVE_PAYMENTS="${HIG_ENABLE_LIVE_PAYMENTS:?Set HIG_ENABLE_LIVE_PAYMENTS explicitly to true or false}"
...
[[ "$API_BASE_URL" == https://* ]] || { echo 'Production API must use HTTPS'; exit 1; }
[[ "$LIVE_PAYMENTS" == true || "$LIVE_PAYMENTS" == false ]] || {
  echo 'HIG_ENABLE_LIVE_PAYMENTS must be true or false'; exit 1;
}
...
for path in /api/v1/readiness /privacy /account-deletion; do
  curl --fail --silent --show-error --max-time 30 "$API_BASE_URL$path" >/dev/null || {
    echo "Production prerequisite unavailable: $API_BASE_URL$path"; exit 1;
  }
done
...
[[ "${HIG_MAP_TILE_URL:-}" == https://* ]] || {
  echo 'HIG_MAP_TILE_URL must be an approved HTTPS production provider'; exit 1;
}
test -n "${HIG_MAP_ATTRIBUTION:-}" || { echo 'HIG_MAP_ATTRIBUTION is required'; exit 1; }
[[ "${HIG_MAP_ATTRIBUTION_URL:-}" == https://* ]] || {
  echo 'HIG_MAP_ATTRIBUTION_URL must use HTTPS'; exit 1;
}

OUT="$REPO/release/play-production-${EXPECTED:0:12}-$BUILD_NUMBER"
case "$OUT" in "$REPO"/release/play-production-*) ;; *) exit 1 ;; esac
...
for APP in staff_admin_app student_parent_app driver_gps_app; do
  (
    ...
    "$FLUTTER" --suppress-analytics build appbundle --release \
      ...
      --dart-define="HIG_ENABLE_LIVE_PAYMENTS=$LIVE_PAYMENTS" \
      --dart-define="HIG_MAP_TILE_URL=$HIG_MAP_TILE_URL" \
      --dart-define="HIG_MAP_ATTRIBUTION=$HIG_MAP_ATTRIBUTION" \
      --dart-define="HIG_MAP_ATTRIBUTION_URL=$HIG_MAP_ATTRIBUTION_URL"
...
echo 'PRODUCTION_AABS=BUILT_NOT_UPLOADED'
