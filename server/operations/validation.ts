...
export const operationActionSchema = z.discriminatedUnion("action", [
  z.object({
    action: z.literal("mark_attendance"),
    studentId: z.string().uuid(),
    attendanceDate: z.iso.date(),
    status: z.enum(["present", "absent", "late", "excused"]),
    note: z.string().trim().max(240).default(""),
    // Required by the mobile service. Optional here so the established web
    // operations contract remains backwards compatible while it is migrated.
