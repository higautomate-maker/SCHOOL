...
export async function applyOperation(tenantId:string, action:OperationAction, actor:ChatGPTUser):Promise<OperationsState>{
  ...
  if(action.action==="mark_attendance"){
    ...
    await env.DB.prepare(`INSERT INTO student_attendance (id,tenant_id,academic_session_id,student_id,attendance_date,status,note,marked_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?) ON CONFLICT(student_id,attendance_date) DO UPDATE SET status=excluded.status,note=excluded.note,marked_by=excluded.marked_by,updated_at=excluded.updated_at`).bind(resourceId,tenantId,session.id,action.studentId,action.attendanceDate,action.status,action.note,actorId,now,now).run();
    ...
    await env.DB.prepare(`INSERT INTO fee_invoices (id,tenant_id,academic_session_id,student_id,fee_type,amount_paise,paid_paise,due_date,status,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?,0,?,'due',?,?,?)`).bind(resourceId,tenantId,session.id,action.studentId,action.feeType,action.amountPaise,action.dueDate,actorId,now,now).run();
  }else{
    const invoice=await env.DB.prepare("SELECT id,student_id AS studentId,amount_paise AS amountPaise,paid_paise AS paidPaise FROM fee_invoices WHERE id=? AND tenant_id=?").bind(action.invoiceId,tenantId).first<{id:string;studentId:string;amountPaise:number;paidPaise:number}>();
    ...
    await env.DB.batch([env.DB.prepare("INSERT INTO fee_payments (id,tenant_id,invoice_id,student_id,amount_paise,method,reference,paid_on,received_by,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)").bind(resourceId,tenantId,invoice.id,invoice.studentId,action.amountPaise,action.method,action.reference,now.slice(0,10),actorId,now),env.DB.prepare("UPDATE fee_invoices SET paid_paise=?,status=?,updated_at=? WHERE id=? AND tenant_id=?").bind(paid,status,now,invoice.id,tenantId)]);
  }
  await env.DB.prepare("INSERT INTO audit_events (id,tenant_id,actor_id,action,resource_type,resource_id,reason,metadata_json,occurred_at) VALUES (?,?,?,?,?,?,'School operations workflow',?,?)").bind(crypto.randomUUID(),tenantId,actorId,`operations.${action.action}`,resourceType,resourceId,JSON.stringify(action),now).run();
...
async function activeSession(tenantId:string){return env.DB.prepare("SELECT id FROM academic_sessions WHERE tenant_id=? AND status='active' ORDER BY starts_on DESC LIMIT 1").bind(tenantId).first<{id:string}>();}
async function requireSchool(tenantId:string){const row=await env.DB.prepare("SELECT id FROM tenants WHERE id=? AND status!='archived'").bind(tenantId).first();if(!row)throw new Error("School not found");}
async function requireStudent(tenantId:string,studentId:string){const row=await env.DB.prepare("SELECT id FROM students WHERE id=? AND tenant_id=?").bind(studentId,tenantId).first();if(!row)throw new Error("Student not found");}
