...
export async function applyRoleAction(tenantId: string, action: RoleAction, actor: ChatGPTUser): Promise<RoleRecord[]> {
  ...
  if (action.action === "create") {
    ...
    const statements = [
      env.DB.prepare("INSERT INTO roles (id, tenant_id, name, key, system, description, created_by, created_at, updated_at) VALUES (?, ?, ?, ?, 0, 'Custom school role', ?, ?, ?)").bind(roleId, tenantId, action.name, key, actorId, now, now),
      ...action.permissions.map((permission) => env.DB.prepare("INSERT INTO role_permissions (role_id, permission, created_at) VALUES (?, ?, ?)").bind(roleId, permission, now)),
    ];
    ...
    const role = await env.DB.prepare("SELECT id, system FROM roles WHERE id = ? AND tenant_id = ?").bind(roleId, tenantId).first<{ id: string; system: number }>();
    ...
    await env.DB.batch([
      ...
      ...action.permissions.map((permission) => env.DB.prepare("INSERT INTO role_permissions (role_id, permission, created_at) VALUES (?, ?, ?)").bind(roleId, permission, now)),
      env.DB.prepare("UPDATE roles SET updated_at = ? WHERE id = ? AND tenant_id = ?").bind(now, roleId, tenantId),
    ]);
    auditAction = "role.permissions_update";
    metadata = { roleId, permissions: action.permissions };
  }

  await env.DB.prepare("INSERT INTO audit_events (id, tenant_id, actor_id, action, resource_type, resource_id, reason, metadata_json, occurred_at) VALUES (?, ?, ?, ?, 'role', ?, 'Permission administration', ?, ?)")
...
async function ensureDefaultRole(tenantId: string, actor: ChatGPTUser): Promise<void> {
  const existing = await env.DB.prepare("SELECT id FROM roles WHERE tenant_id = ? AND key = 'school_admin'").bind(tenantId).first<{ id: string }>();
  if (existing) return;
  const school = await env.DB.prepare("SELECT id FROM tenants WHERE id = ? AND status != 'archived'").bind(tenantId).first<{ id: string }>();
  if (!school) throw new Error("School not found");
  const now = new Date().toISOString();
  ...
  await env.DB.batch([
    env.DB.prepare("INSERT INTO roles (id, tenant_id, name, key, system, description, created_by, created_at, updated_at) VALUES (?, ?, 'School Admin', 'school_admin', 1, 'Full tenant administration', ?, ?, ?)").bind(roleId, tenantId, actorId, now, now),
    ...permissionCatalogue.map(([permission]) => env.DB.prepare("INSERT INTO role_permissions (role_id, permission, created_at) VALUES (?, ?, ?)").bind(roleId, permission, now)),
  ]);
}
