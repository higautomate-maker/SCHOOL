...
export async function getWorkspace(tenantId:string,moduleKey:string,sessionId?:string|null):Promise<WorkspaceState>{
  ...
  const statement=env.DB.prepare(`SELECT id,module_key AS moduleKey,workflow,title,description,record_date AS recordDate,due_date AS dueDate,amount_paise AS amountPaise,assignee,priority,status,created_at AS createdAt,updated_at AS updatedAt FROM module_records WHERE ${conditions.join(" AND ")} ORDER BY CASE priority WHEN 'urgent' THEN 1 WHEN 'high' THEN 2 WHEN 'normal' THEN 3 ELSE 4 END, updated_at DESC LIMIT 500`);
  const records=(await statement.bind(...values).all<ModuleRecord>()).results;
  const breakdown=(await env.DB.prepare(`SELECT module_key AS moduleKey,COUNT(*) AS total,SUM(CASE WHEN status IN ('open','in_progress') THEN 1 ELSE 0 END) AS openCount,SUM(CASE WHEN status='completed' THEN 1 ELSE 0 END) AS completedCount FROM module_records WHERE tenant_id=? GROUP BY module_key ORDER BY total DESC`).bind(tenantId).all<ModuleBreakdown>()).results;
  const today=new Date().toISOString().slice(0,10);return {records,breakdown,metrics:{total:records.length,open:records.filter(r=>r.status==="open").length,inProgress:records.filter(r=>r.status==="in_progress").length,completed:records.filter(r=>r.status==="completed").length,urgent:records.filter(r=>r.priority==="urgent"&&r.status!=="completed"&&r.status!=="cancelled").length,overdue:records.filter(r=>Boolean(r.dueDate&&r.dueDate<today)&&r.status!=="completed"&&r.status!=="cancelled").length,amountPaise:records.reduce((n,r)=>n+(r.amountPaise??0),0)}};
}
...
export async function applyWorkspaceAction(tenantId:string,action:WorkspaceAction,actor:ChatGPTUser):Promise<WorkspaceState>{
  ...
  if(action.action==="create_record") {const session=await activeSession(tenantId);resourceId=crypto.randomUUID();moduleKey=action.moduleKey;await env.DB.prepare(`INSERT INTO module_records (id,tenant_id,academic_session_id,module_key,workflow,title,description,record_date,due_date,amount_paise,assignee,priority,status,metadata_json,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?, 'open','{}',?,?,?)`).bind(resourceId,tenantId,session?.id??null,action.moduleKey,action.workflow,action.title,action.description,action.recordDate,action.dueDate||null,action.amountPaise,action.assignee,action.priority,actorId,now,now).run();}
  else {const record=await env.DB.prepare("SELECT id,module_key AS moduleKey FROM module_records WHERE id=? AND tenant_id=?").bind(action.recordId,tenantId).first<{id:string;moduleKey:string}>();if(!record)throw new Error("Workspace record not found");resourceId=record.id;moduleKey=record.moduleKey;await env.DB.prepare("UPDATE module_records SET status=?,updated_at=? WHERE id=? AND tenant_id=?").bind(action.status,now,action.recordId,tenantId).run();}
  await env.DB.prepare("INSERT INTO audit_events (id,tenant_id,actor_id,action,resource_type,resource_id,reason,metadata_json,occurred_at) VALUES (?,?,?,?,?,?,'Module workflow operation',?,?)").bind(crypto.randomUUID(),tenantId,actorId,`workspace.${action.action}`,"module_record",resourceId,JSON.stringify(action),now).run();
