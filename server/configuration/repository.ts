...
export async function getConfiguration(tenantId: string) {
  ...
  const rows = await env.DB.prepare("SELECT config_key AS configKey, payload_json AS payloadJson FROM school_configurations WHERE tenant_id = ?").bind(tenantId).all<{ configKey:string;payloadJson:string }>();
...
export async function applyConfigurationAction(tenantId: string, action: ConfigurationAction, actor: ChatGPTUser) {
  ...
  const existing = await env.DB.prepare("SELECT payload_json AS payloadJson FROM school_configurations WHERE tenant_id = ? AND config_key = 'payment_gateway'").bind(tenantId).first<{ payloadJson: string }>();
