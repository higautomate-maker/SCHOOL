...
async function getSqliteSchoolDetail(tenantId: string): Promise<SchoolDetail | null> {
  ...
  const [modules, audit] = await Promise.all([
    database.prepare(`SELECT module_key, enabled FROM module_policies WHERE tenant_id = ? ORDER BY module_key`).bind(tenantId).all<ModuleRow>(),
    database.prepare(`SELECT id, action, occurred_at, metadata_json FROM audit_events WHERE tenant_id = ? ORDER BY occurred_at DESC LIMIT 8`).bind(tenantId).all<AuditRow>(),
  ]);
  return {
    tenantId: school.id,
    name: school.name,
...
export async function performSchoolAction(tenantId: string, action: SchoolAction, actor: ChatGPTUser, idempotencyKey: string): Promise<SchoolDetail> {
  ...
  if (action.action === "update_plan") {
    ...
    statements.push(
      ...
      database.prepare(`UPDATE subscriptions SET plan_id = ?, updated_at = ? WHERE tenant_id = ?`).bind(planId, nowIso, tenantId),
    );
    ...
    statements.push(database.prepare(`UPDATE school_invitations SET status = 'revoked', updated_at = ? WHERE tenant_id = ?`).bind(nowIso, tenantId));
  ...
  await database.prepare(`INSERT INTO idempotency_records (key, actor_email, operation, response_json, created_at, expires_at)
